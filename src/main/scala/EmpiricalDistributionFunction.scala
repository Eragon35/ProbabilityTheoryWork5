...
object EmpiricalDistributionFunction {
  def show(func: ArrayBuffer[Double]): Unit = {
    ...
    for(i <- 1 until func.size) {
      // строчка по середине
      if (i == func.size / 2 + 1) println(s" F* ${func.size}(х) = | ${i.toDouble / func.size}, ${func(i-1)} < x <= ${func(i)}")
      println(s"$tabs| ${i.toDouble / func.size}, ${func(i-1)} < x <= ${func(i)}") // все остальные строчки
    }
    ...
    for(i <- 1 until 20) dataset = dataset :+ (s"${func(i - 1)} to ${func(i)}", // название прямой в данной случае значение точки
